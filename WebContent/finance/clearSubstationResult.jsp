			...
			<div id="page-wrapper" style="background: #f2f2f2;min-width: 800px;  min-height: 299px;">
				...
				<div style="display: flex;flex-direction: row;align-items:flex-start;">
					<div class="homeblock3" style="margin-left:auto; margin-right:auto;">
						...
						<form action="../clearMoneyServlet?action=selectSign" method="post" role="form">
				...
				<!--/# 搜索 -->
					...
					<div class="searchResult">
						...
						<c:forEach items="${resultListselectSign}" var="sign">
							<div class="searchItem">
							<p class="p20"><input type="checkbox" value="${sign.signId}" name="chk"></p>
						...
						<div class="page-turner ">
							<ul class="pagination pagination-lg ">
								<li <c:if test="${pageselectSign eq 1}">class="disabled "</c:if>>
									<a href="<c:if test="${pageselectSign != 1}">../clearMoneyServlet?action=selectSignByPage&page=${pageselectSign-1}</c:if>"><i class="fa fa-angle-left "></i></a>
								</li>
								<c:forEach begin="1" end="${pageNumselectSign}" var="p"  >
									<c:if test="${p==pageselectSign}">
										<li class="active ">
											<a href=" ">${p}</a>
										</li>
									</c:if>
									<c:if test="${p!=pageselectSign}">
										<li>
											<a href="../clearMoneyServlet?action=selectSignByPage&page=${p}"  >${p}</a>
										</li>
									</c:if>
									&nbsp;&nbsp;
								</c:forEach>
								<li <c:if test="${pageselectSign eq pageNumselectSign}">class="disabled "</c:if>>
									<a href="<c:if test="${pageselectSign < pageNumselectSign}">../clearMoneyServlet?action=selectSignByPage&page=${pageselectSign+1}</c:if>"><i class="fa fa-angle-right "></i></a>
								</li>
								<li><button class="buttonjiesuan" type="submit" onclick="clearSubstation()">结算</button></li>
								</ul>
							</div>
						</div>
					</div>
