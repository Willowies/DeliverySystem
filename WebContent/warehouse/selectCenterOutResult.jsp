		...
		<div id="wrapper">
			...
			<div id="page-wrapper" style="background: #f2f2f2;min-width: 800px;  min-height: 299px;">
				<div class="page-navigation">
					<strong>查询出库信息 / Query outbound information</strong>
				</div>
				<!-- 搜索 -->
				<div style="display: flex;flex-direction: row;align-items:flex-start;">
					...
					<div class="searchResult">
						...
						<c:forEach items="${resultList}" var="centerOutRecord">
							<div class="searchItem">
							<p class="p70">${centerOutRecord.productId}</p>
							<p class="p70">${centerOutRecord.productName}</p>							
							<p class="p70">${centerOutRecord.price}</p>
							<p class="p70">${centerOutRecord.productQuantity}</p>
							<p class="p70">${centerOutRecord.manufacturer}</p>
							<p class="p70">${centerOutRecord.remark}</p>
							</div>
						</c:forEach>
					
						<div class="pageFoot">
							<ul class="pagination" style="margin:10px 0;">
						        <li <c:if test="${page1 eq 1}">class="disabled "</c:if>>
									<a href="<c:if test="${page1 != 1}">warehouseManageServlet?action=selectCenterOutByPage&page=${page1-1}</c:if>"><i class="fa fa-angle-left "></i></a>
								</li>
								<c:forEach begin="1" end="${pageNum1}" var="p"  >
									<c:if test="${p==page1}">
										<li class="active ">
											<a href=" ">${p}</a>
										</li>
									...
									<c:if test="${p!=page1}">
										<li>
											<a href="warehouseManageServlet?action=selectCenterOutByPage&page=${p}"  >${p}</a>
										</li>
									</c:if>
									&nbsp;&nbsp;
								</c:forEach>
								<li <c:if test="${page1 eq pageNum1}">class="disabled "</c:if>>
									<a href="<c:if test="${page1 < pageNum1}">warehouseManageServlet?action=selectCenterOutByPage&page=${page1+1}</c:if>"><i class="fa fa-angle-right "></i></a>
								</li>
						    </ul>
						</div>
