...
<head>
		...
		<!--
			弹窗的js
        -->
		<script src="<%=basePath%>/js/jumpout.js"></script>
	</head>
	...
	<body>
		<div id="wrapper">
			...
			<div id="page-wrapper" style="background: #f2f2f2;display: flex;">				
				...
				<form action="warehouseServlet?action=selectWarehouse" method="post">
					<div class="page-navigation">查询库房 / Query the warehouse</div>
					<div class="increaseWarehouse2">
						<p1>查询库房</p1>
						<p>库房名称</p>
						<input type="text" name="warehouseName" >
						<div class="buttonblock">
							<button type="submit">搜索</button>
							<button type="button" onclick="window.location.href='warehouse1/setWarehouse.jsp'">取消</button>
						</div>
					</div>
				</form>
				
				<div class="selectResult" id="data">
					<table class="table">
						<tr>
							<th>库房名称</th>
							<th>库房地址</th>
						...
						<c:forEach items="${resultList}" var="warehouse">
						<tr>
							<td>${warehouse.warehouseName}</td>
							<td>${warehouse.warehouseAddress}</td>
							<td>${warehouse.warehouseKeeper}</td>
							<td>${warehouse.warehouseRank}</td>
							<!-- <td><a href="warehouseServlet?warehouseId=${warehouse.warehouseId}&action=editWarehouse" >修改</a></td>	-->		
							<td><a href="warehouseServlet?warehouseId=${warehouse.warehouseId}&action=deleteWarehouse" >删除</a></td>			
							</tr>
						</c:forEach>
					</table>
