	...
	<body>
		<div id="wrapper">
			...
			<div id="page-wrapper" style="background: #f2f2f2;display: flex;">
				<form action="./deliveryPartServlet?action=searchReturnProduct" method="post">
					<div class="page-navigation">退货管理  / Stock </div>
					<div class="searchblock4" style="height: 320px;width: 240px;">
						<p1>退货查询</p1>
						<p>供应商</p>
						<input  type="text" name="supName"  style="width: 200px;">
						<p>商品编号</p>
						<input  type="text" name="productCode"  style="width: 200px;">
						<p>开始日期</p>
				...
				<div class="resultblock2">
					...
					<form action="./deliveryPartServlet?action=returnProduct" method="post" style="flex: 1;display: flex;flex-direction: column;">
					<div class="result-content" >
						...
						<c:forEach items="${orderList}" var="order">
							<div class="result-item">
								...
								<p3 style="margin-left: 10px;width: 100px;">${order.supName}</p3>
								<p3 style="margin-left: 10px;width: 60px;">${order.productCode}</p3>
								<p3 style="margin-left: 10px;width: 120px;">${order.productName}</p3>
								<p3 style="margin-left: 20px;width: 80px;">${order.stock}</p3>
								<p3 style="margin-left: 20px;width: 80px;">${order.now}</p3>
								<p3 style="margin-left: 50px;">${order.returnQ}</p3>
								<p3 style="width: 70px;margin-left: 80px;">
									<input type="number" name="productQuantity${order.productId}" value="${order.returnQ}" style="width: 50px;" min="1" max="10000">
								</p3>
							</div>
						</c:forEach>
						<%
					...
					<div class="result-foot" style="display: flex;">
						<div class="page-turner">
							<ul class="pagination pagination-lg">
								<li <c:if test="${page eq 1}">class="disabled "</c:if>>
									<a href="<c:if test="${page != 1}">./deliveryPartServlet?action=searchReturnProductByPage&page=${page-1}</c:if>"><i class="fa fa-angle-left "></i></a>
								</li>
								<c:forEach begin="1" end="${returnPageNum}" var="p"  >
									<c:if test="${p==page}">
										<li class="active ">
											<a href=" ">${p}</a>
										</li>
									...
									<c:if test="${p!=page}">
										<li>
											<a href="./deliveryPartServlet?action=searchReturnProductByPage&page=${p}"  >${p}</a>
										</li>
									</c:if>
									&nbsp;&nbsp;
								</c:forEach>
								<li <c:if test="${page eq returnPageNum}">class="disabled "</c:if>>
									<a href="<c:if test="${page < returnPageNum}">./deliveryPartServlet?action=searchReturnProductByPage&page=${page+1}</c:if>"><i class="fa fa-angle-right "></i></a>
								</li>
							</ul>
						</div>
						<%
						...
						<div class="operate-button" style="margin: 0 auto;">
							<button class="b" type="submit">退货</button>
						</div>
						<%}%>
					</div>
					</form>
