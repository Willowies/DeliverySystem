	...
	<body>
		<div id="wrapper">
				...
				<div class="navbar-default sidebar" role="navigation">
					<div class="sidebar-nav navbar-collapse">
						<ul class="nav" id="side-menu">
							...
							<li>
								<a href="./delivery/deliveryHome.jsp"><i class="fa fa-download nav_icon"></i>商品管理<span class="fa arrow"></span></a>
			...
			<div id="page-wrapper" style="background: #f2f2f2;display: flex;">
				<form action="./deliveryPartServlet?action=substationStatistics" method="post">
					<div class="page-navigation">统计  / Statistics </div>
					<div class="searchblock4" style="height: 280px;width: 240px;">
						<p1>分站统计</p1>
						<p>开始日期</p>
						<input  type="date" name="start2" id="ipt1" style="width: 200px;" required="required" onblur="date2()">
						<p>结束日期</p>
						<input  type="date" name="end2" id="ipt2" style="width: 200px;" required="required" onblur="dateEnd2()">
				...
				<div class="resultblock2">
					<div class="title">
						<p3 style="margin-left: 100px;">分站名称</p3>
						<p3 style="margin-left: 40px;">执行任务数</p3>
						<p3 style="margin-left: 40px;">送货商品数</p3>
						<p3 style="margin-left: 10px;">送货金额</p3>
					</div>
					...
					<div class="result-content" >
						...
						<c:forEach items="${orderList}" var="order">
							<div class="result-item">
								<p3 style="margin-left: 120px;width: 140px;">${order.warehouseName}</p3>
								<p3 style="margin-left: 10px;width: 100px;">${order.orderQ}</p3>
								<p3 style="margin-left: 10px;width: 120px;">${order.quantity}</p3>
								<p3 style="margin-left: 10px;width: 40px;">${order.total}</p3>
							</div>
						</c:forEach>
						<%
						}else{
						...
						<div class="page-turner">
							<ul class="pagination pagination-lg">
								<li <c:if test="${page eq 1}">class="disabled "</c:if>>
									<a href="<c:if test="${page != 1}">./deliveryPartServlet?action=substationStatisticsByPage&page=${page-1}</c:if>"><i class="fa fa-angle-left "></i></a>
								</li>
								<c:forEach begin="1" end="${substationStatisticsPageNum}" var="p"  >
									<c:if test="${p==page}">
										<li class="active ">
											<a href=" ">${p}</a>
										</li>
									...
									<c:if test="${p!=page}">
										<li>
											<a href="./deliveryPartServlet?action=substationStatisticsByPage&page=${p}"  >${p}</a>
										</li>
									</c:if>
									&nbsp;&nbsp;
								</c:forEach>
								<li <c:if test="${page eq substationStatisticsPageNum}">class="disabled "</c:if>>
									<a href="<c:if test="${page < substationStatisticsPageNum}">./deliveryPartServlet?action=substationStatisticsByPage&page=${page+1}</c:if>"><i class="fa fa-angle-right "></i></a>
								</li>
							</ul>
						</div>
					</div>
