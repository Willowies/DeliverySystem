	...
	<body>
		<div id="wrapper">
			...
			<div id="page-wrapper" style="background: #f2f2f2;display: flex;">
				<form action="./deliveryPartServlet?action=searchWarehouseValue" method="post">
					<div class="page-navigation">库存量查询  / Warehouse value </div>
					<div class="searchblock4" style="height: 250px;width: 240px;">
						<p1>库存量查询</p1>
						<p>商品名称</p>
						<input  type="text" name="productName" style="width: 200px;">
						<p>库房名称</p>
						<input  type="text" name="warehouseName"  style="width: 200px;" >
				...
				<div class="resultblock2">
					...
					<div class="result-content">
						...
						<div class="result-item" style="margin-top: 20px;">
							<p3 style="margin-left: 20px;width: 110px;">${order.warehouseName}</p3>
							<p3 style="margin-left: 10px;width: 110px;">${order.productName}</p3>
							<p3 style="margin-left: 10px;width: 80px;">${order.total}</p3>
							<p3 style="margin-left: 20px;width: 50px;">${order.returnQ}</p3>
							<p3 style="margin-left: 20px;width: 80px;">${order.allocate}</p3>
							<p3 style="margin-left: 40px;">${order.allocated}</p3>
						</div>
						</c:forEach>
						<%
						}else{
						...
						<div class="page-turner">
							<ul class="pagination pagination-lg">
								<li <c:if test="${page eq 1}">class="disabled "</c:if>>
									<a href="<c:if test="${page != 1}">./deliveryPartServlet?action=searchWarehouseValueByPage&page=${page-1}</c:if>"><i class="fa fa-angle-left "></i></a>
								</li>
								<c:forEach begin="1" end="${warehouseValuePageNum}" var="p"  >
									<c:if test="${p==page}">
										<li class="active ">
											<a href=" ">${p}</a>
										</li>
									...
									<c:if test="${p!=page}">
										<li>
											<a href="./deliveryPartServlet?action=searchWarehouseValueByPage&page=${p}"  >${p}</a>
										</li>
									</c:if>
									&nbsp;&nbsp;
								</c:forEach>
								<li <c:if test="${page eq warehouseValuePageNum}">class="disabled "</c:if>>
									<a href="<c:if test="${page < warehouseValuePageNum}">./deliveryPartServlet?action=searchWarehouseValueByPage&page=${page+1}</c:if>"><i class="fa fa-angle-right "></i></a>
								</li>
							</ul>
						</div>
					</div>
