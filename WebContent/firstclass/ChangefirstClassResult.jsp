	...
	<body>
		<div id="wrapper">
			...
			<div id="page-wrapper" style="background: #f2f2f2;display: flex;">
				<form action="<%=basePath%>/firstClassMangerServlet?action=selectfirstclass" method="post">
					<div class="page-navigation">编辑一级分类 / Change FirstClass</div>
					<div class="searchblock2">
						<p1>搜索分类</p1>
						<p>分类名称</p>
						<input type="text " name="firstclassname" id="firstclassname"placeholder="必填" required="required">
						<p>分类ID</p>
						<input type="number" name="firstclassid" id="firstclassid"placeholder="必填" required="required">
				...
				var属性：当前遍历到的元素
			    -->
						<c:forEach items="${firstclassresultlist}" var="firstclass" >
							<tr>
								<td>
									<input type="checkbox" value="${firstclass.firstClassId}" name="chk">
								</td>
								<td>${firstclass.firstClassId}</td>	
								<td>${firstclass.firstClassName}</td>
								<td>${firstclass.firstClassDescription}</td>
															
								<td>
									<a href="firstClassManagerServlet?firstClassId=${firstclass.firstClassId}&action=edit">编辑</a>
								</td>
							</tr>
						</c:forEach>
						</table>
					...
					<div class="result-foot">
						...
						<div class="operate-button">
							<button class="b" onclick="deletefirstclass()">删除</button>
						</div>
					</div>
